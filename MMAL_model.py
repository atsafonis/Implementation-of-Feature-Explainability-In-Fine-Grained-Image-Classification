...
class MainNet(nn.Module):
    ...
    '''
    def old_forward(self, x, status='test', DEVICE='cuda'):
        fm, embedding, conv5_b = self.pretrained_model(x)
        id = self.shortcut(conv5_b)
        batch_size, channel_size, side_size, _ = fm.shape
        assert channel_size == 2048
        ...
        #SCDA
        coordinates = torch.tensor(AOLM(fm.detach(), id.detach()))
    ...
    def new_forward(self, x, status='test', DEVICE='cuda'):
        fm, embedding, conv5_b = self.pretrained_model(x)
        id = self.shortcut(conv5_b)
        batch_size, channel_size, side_size, _ = fm.shape
        assert channel_size == 2048
    ...
    def data_func(self, x):
        fm, embedding, conv5_b = self.pretrained_model(x)

        return fm, embedding, conv5_b


    def forward_local_logits(self, x, fm, embedding, conv5_b):
    ...
    def feature_for(self, x, status='test', DEVICE='cuda'):
        fm, embedding, conv5_b = self.pretrained_model(x)
        id = self.shortcut(conv5_b)
    
        return  id
    
    def feature_map_for(self, x, status='test', DEVICE='cuda'):
        fm, embedding, conv5_b = self.pretrained_model(x)
